...
def add_to_cart():
    ...
    if qty <= 0:
        ...
        return
    if qty > stock:
        print(f" Requested quantity exceeds available stock ({stock}).")
        return
...
def view_cart():
    ...
    for (n, b), qty in cart_mgr.cart.items():
        price = None
        for p, name, brand, _ in products_heap.heap:
    ...
    while heap_list:
        ...
        total += subtotal
    print(f"Total (current prices): Rs{total:.2f}")
...
def modify_cart():
    ...
    if (name, brand) not in cart_mgr.cart:
        found = [(n, b) for (n, b) in cart_mgr.cart.keys()
                 if n == name and b.lower() == brand.lower()]
        if found:
            name, brand = found[0]
        else:
